...
def privacy_policy(request):
    ...
    if page:
        ...
        return render(request, 'blog/static_page.html', {
            'page': page, 'title': page.get_title(lang), 'content': page.get_content(lang),
        })
...
def faq(request):
    from blog.models import StaticPage
    from django.utils.translation import get_language
    page = StaticPage.objects.filter(page_type='faq', is_published=True).first()
    if page:
        lang = get_language() or 'lv'
        return render(request, 'blog/static_page.html', {
            'page': page, 'title': page.get_title(lang), 'content': page.get_content(lang),
        })
    return render(request, 'listings/faq.html', {'faqs': [], 'quick_links': []})
